...
def _variable_on_device(name,shape,initializer, trainable = True):
    ...
    if not callable(initializer):
        ...
        var = tf.get_variable(
            name,shape,initiallizer =initializer,dtype = dtype, trainable = trainable)
...
def _variable_with_weight_decay(name,shape,wd,initializer,trainable = True):
    ...
    if wd is not None and trainable:
        weight_decay = tf.multiply(tf.nn.l2_loss(var),wd, name = 'weight_loss')  ##tf.nn.l2_loss(t, name=None) ->output = sum(t ** 2) / 2
        tf.add_to_collection('losses', weight_decay)  ## element 'weight_decay' ->list 'losses'
    return var
...
class ModelSkeleton(object):
    ...
    def __init__(self,mc):
        ...
        #a scalor tensor in range (0,1]. Usually set to 0.5 in training phase and 1.0 in evaluation phase
        ...
        # A tensor where an element is 1 if the corresponding cell contains an valid measurement.
        # Or if the data is missing, then mask it as 0.
        self.ph_lidar_mask = tf.placeholder(
            tf.float32, [mc.BATCH_SIZE, mc.ZENITH_LEVEL, mc.AZIMUTH_LEVEL, 1],
            name ='lidar_mask'
        )
        self.ph_lable = tf.placeholder(
        ...
        self.enqueue_op = self.q.enqueue(
            [self.ph_keep_prob, self.ph_lidar_input, self.ph_lidar_mask, self.ph_lable,
             self.ph_loss_weight]
        )

        self.keep_prob, self.lidar_input, self.lidar_mask, self.lable, self.loss_weight = self.q.dequeue()

        #model parameters
        self.model_params =[]
        # model size counter  #? TODO
        self.model_size_counter = []
        #flop counter #? TODO
        self.flop_counter =[]
        #activate counter #? TODO
    ...
    def _deconv_layer(self, layer_name, inputs, filters, size, stride, padding='SAME',
        ...
        if mc.DEBUG_MODE:
            print('Input tensor shape to {}:{}'.format(layer_name, input.get_shape()))
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            if init == 'xavier':
                ...
                'In bilinear iterporlation mode ,input channel size und output'\
                'filter size should be the same'
                assert stride_h ==1, \
                'in bilinear interporlation mode, stride_h should be 1'

                kernel_init = np.zeros(
                    (size_h, size_w, channels, channels), dtype=np.float32)
                ...
                og_w = np.reshape(np.arange(size_w),(size_h, -1)) ##? TODO
            ...
            #Kernel layout for deconv layer :[H f, W f,O f, I c] where I c is the Input channel size.
            #it would be the same as the channel size of the input tensor.
            kernel = _variable_with_weight_decay(
            ...
            deconv = tf.nn.conv2d_transpose(
                inputs, kernel, [mc.BATCH_SIZE, stride_h*in_height, stride_w*in_width, filters],
            ...
            out_shape = out.get_shape[].as_list()
            num_flops = \
                (1+2*channels*size_h*size_w)*filters*out_shape[1]*out_shape[2]  # ?TODO
            if relu:
                num_flops += 2*filters*out_shape[1]*out_shape[2]
            self.flop_counter.append((layer_name, num_flops))
    ...
    def _bilateral_filter_layer(
        ...
        size_z, size_a =sizes
        batch, zenith, azimute, in_channel = inputs.shape.as_list()
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            diff_x = tf.reshape(inputs[:, :, :, 0], [batch, zenith, azimute, 1]) \
                - condensed_input[:, :, :, 0::in_channel]
            diff_y = tf.reshape(inputs[:, :, :, 1], [batch, zenith, azimute, 1]) \
                - condensed_input[:, :, :, 1::in_channel]
            diff_z = tf.reshape(inputs[:, :, :, 2], [batch, zenith, azimute, 1]) \
                - condensed_input[:, :, :, 2::in_channel]

            bi_filters = []
            for cls in range(mc.NUM_CLASS):
                theta_a = mc.BILATERAL_THETA_A[cls]
                theta_r = mc.BILETERAL_THETA_R[cls]
    ...
    def _recurrent_crf_layer(
        ...
        with tf.variable_scope(layer_name) as scope:
            ...
            self._activation_summary(bi_compat_kernel,'bilatreal_compat_mat')
            ...
            for it in range(num_iterations):
                unary = tf.nn.softmax(
                    inputs, dim=-1, name='unary_term_at_iter_{}'.foramt(it)
                )
                ...
                pairwise = tf.add(ang_output, bi_output, name='pairwise_term_at_iter_{}'.format(it))

                outputs = tf.add(unary, pairwise, name='energy_at_iter_{}'.format(it))
    ...
    def _locally_connected_layer(
        ...
        assert sizes[0] %2 == 1 and sizes[1]%2 == 1, 'Currently only support odd filter size.'
