...
class Rsnet(ModelSkeleton):
    ...
    def _add_forward_graph(self):
        ...
        conv1_skip = self._conv_layer(
            'conv1_skip',self.lidir_input, filters=64, size = 1, stride = 1,
            padding = 'SAME', freeze=False, xavier=True)
        ...
        fire7 = self._fire_layer(
            'frie7', fire6, s1x1=48, e1x1=192,e3x3=192, freeze=False)
        ...
        fire10 = self._fire_deconv(
            ...
            stddev=0.1)
        fire10_fuse = tf.add(fire10, fire5, name='fire10_fuse')
        ...
        drop13 = tf.nn.dropout(fire13_fuse, self.keep_porb, name='drop13')

        conv14 = self._conv_layer(
            'con14_prob', drop13, filters=mc.NUM_CLASS, size=3, stride=1,
            padding='SAME', relu=False, stddev=0.1)

        bilateral_filter_weights = self._bilateral_filter_layer(
            'bitateral_filter', self.lidar_input[:, :, :, :3],#x,y,z
    ...
    def _fire_layer(self,layer_name, inputs, s1x1,e1x1, e3x3, stddev=0.001,freeze=False):
        ...
        """
        sq1x1 = self._conv_layer(layer_name+'/squeeze1x1', inputs, filters=s1x1,size =1, stride=1,
                                 padding ='SAME', freeze=freeze, stddev=stddev)
        ex1x1 = self._conv_layer(layer_name+'/expand1x1', sq1x1, filter=e1x1, size =1 ,stride =1,
                                 padding ='SAME', freeze=freeze,stddev=stddev)
        ex3x3 = self._conv_layer(layer_name+'/expand3x3', sq1x1, filter=e3x3,size=3, stride =1,
                                 padding ='SAME',freeze=freeze, stddev=stddev)
    ...
    def _fire_deconv(self,layer_name,inputs, s1x1, e1x1, e3x3, factors=[1, 2],freeze=False,stddev=0.001 ):
        ...
        'factors should be an array of size 2'
